...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-white/10 py-3 last:border-0">
      <span className="text-[11px] font-semibold uppercase tracking-[0.14em] text-[#c8c8c8]">
        {label}
      </span>
  ...
  return (
    <aside className="lg:sticky lg:top-6">
      <div className="brand-aside">
        ...
        {summary.capabilities.length > 0 ? (
          <div className="mt-4 flex flex-wrap gap-1.5">
            ...
            {summary.capabilities.length > 8 ? (
              <span className="rounded-full bg-white/5 px-2.5 py-1 text-[10px] text-[#a8a8a8] ring-1 ring-white/10">
                +{summary.capabilities.length - 8}
              </span>
            ) : null}
          </div>
        ...
        <div className="mt-7">
          <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#c8c8c8]">
            ...
            <AnimatePresence mode="popLayout">
              {recs.length === 0 ? (
                <motion.p
                  ...
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="rounded-2xl bg-white/5 px-4 py-5 text-sm text-[#c8c8c8] ring-1 ring-white/10"
                >
                ...
                recs.map((rec) => (
                  <motion.div
                    ...
                    className="rounded-2xl bg-[#2c2c2c] px-4 py-3 ring-1 ring-white/10"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-0.5 text-[11px] text-[#c8c8c8]">
                          {rec.product.tagline}
                        </p>
                      </div>
