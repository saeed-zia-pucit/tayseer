...
export function ResultsPage() {
  ...
  return (
    <ExperimentShell contentClassName="mx-auto max-w-5xl px-5 pb-20 pt-6 md:px-8">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {solution.products.map((rec, index) => (
            <motion.article
              ...
              className="rounded-[1.5rem] bg-[#161616] p-6 shadow-lift ring-1 ring-white/10"
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 text-sm text-[#a8a8a8]">{rec.product.tagline}</p>
                </div>
              ...
              <ul className="mt-4 space-y-1.5">
                {rec.reasons.map((reason) => (
                  <li key={reason} className="flex gap-2 text-xs text-[#a8a8a8]">
      ...
      <section className="mt-14">
        ...
        <div className="mt-6 overflow-x-auto rounded-[1.5rem] bg-[#121212] p-6 text-white shadow-lift ring-1 ring-white/10 md:p-8">
          <div className="flex min-w-[36rem] items-center justify-between gap-2">
            {solution.architecture.map((node, i) => (
              <div key={`${node}-${i}`} className="flex flex-1 items-center gap-2">
                <div className="w-full rounded-2xl bg-[#222222] px-3 py-4 text-center ring-1 ring-white/10">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-lagoon-bright">
                    Layer {i + 1}
                  </p>
                  <p className="mt-2 text-sm font-semibold leading-snug text-white">{node}</p>
                </div>
      ...
      <section className="mt-14 grid gap-6 md:grid-cols-[1.2fr_1fr]">
        <div className="rounded-[1.5rem] bg-[#161616] p-6 ring-1 ring-white/10 md:p-8">
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#a8a8a8]">
